...
angular.module(ApplicationConfiguration.applicationModuleName, ApplicationConfiguration.applicationModuleVendorDependencies), angular.module(ApplicationConfiguration.applicationModuleName).config(["$locationProvider", function($locationProvider) {
...
}]), angular.module("accounts", ["fiestah.money"]).controller("AccountsController", ["$scope", "$modal", "$log", "$stateParams", "$location", "Authentication", "Accounts", "$http", "Organizations", function($scope, $modal, $log, $stateParams, $location, Authentication, Accounts, $http, Organizations) {
    $scope.authentication = Authentication, $scope.myData = [], $scope.myData = Accounts.query(), $scope.mySelections = [], $scope.items = [], $scope.selected = "", $scope.shouldBeDisabled = !0, $scope.accountOwner__C = $scope.authentication.user.displayName, $scope.date__C = new Date, $scope.filterOptions = {
        filterText: "",
        useExternalFilter: !0
        ...
        formatYear: "yy",
        startingDay: 1
    }, $scope.formats = ["dd-MMMM-yyyy", "yyyy/MM/dd", "dd.MM.yyyy", "shortDate"], $scope.format = $scope.formats[0], $scope.setPagingData = function(data, page, pageSize) {
    ...
    }, !0), $scope.onlyAccounts = Accounts.query(function(result) {
        for (var x = (result.length, 0); x < result.length; x++) result[x].user._id == $scope.authentication.user._id && $scope.items.push(result[x]);
        return console.log("$scope.items ==", $scope.items), $scope.items
    }), $scope.gridOptions = {
        data: "items",
        enablePaging: !0,
        showFooter: !0,
        ...
        $scope.AllCompanies = Organizations.query(), $scope.organization = Organizations.get({
        ...
        modalInstance.result.then(function(selectedItem) {
            $scope.selected = selectedItem, console.log(" $scope.selected", $scope.selected)
        }, function() {
            $log.info("Modal dismissed at: " + new Date)
        })
    }, $scope.copyBaddressToSaddress = function() {
        $scope.enable = !0, 1 == $scope.enable && ($scope.Saddress = $scope.Baddress), $scope.enable = !1
    }, $scope.create = function() {
        if (console.log(" $scope.selected in create method", $scope.selected), $scope.parentAccount = [], angular.isUndefined(this.Baddress) && ($scope.Baddress = ""), angular.isUndefined(this.id__C)) {
            ...
            account.$save(function() {
                $scope.rating__C = "", $scope.phone__C = "", $scope.accountName__C = "", $scope.selected = "", $scope.fax__C = "", $scope.accountNumber__C = "", $scope.website__C = "", $scope.accountSite__C = "", $scope.type__C = "", $scope.annualRevenue__C = "", $scope.ownership__C = "", $scope.employees__C = "", $scope.industry__C = "", $scope.Baddress = "", $scope.Saddress = "", $scope.Description_C = ""
            }, function(errorResponse) {
                console.log("account is not created"), $scope.error = errorResponse.data.message, alert($scope.error)
            })
        } else {
            var account = new Accounts({
...
}]), angular.module("contacts").controller("ContactsController", ["$scope", "$stateParams", "$location", "Authentication", "Contacts", "$http", function($scope, $stateParams, $location, Authentication, Contacts, $http) {
    $scope.authentication = Authentication, $scope.myData = [], $scope.myData = Contacts.query(), $scope.mySelections = [], $scope.perContact = [], $scope.date__C = new Date, $scope.filterOptions = {
        filterText: "",
        useExternalFilter: !0
    }, $scope.totalServerItems = 0, $scope.pagingOptions = {
        pageSizes: [250, 500, 1e3],
        ...
        newVal !== oldVal && $scope.getPagedDataAsync($scope.pagingOptions.pageSize, $scope.pagingOptions.currentPage, $scope.filterOptions.filterText)
    }, !0), $scope.onlyContacts = Contacts.query(function(result) {
        console.log("result ==", result);
        var len = result.length;
        console.log("length", +len);
        for (var x = 0; x < result.length; x++) result[x].user._id == $scope.authentication.user._id && $scope.perContact.push(result[x]);
        return console.log("$scope.perContact ==", $scope.perContact), $scope.perContact
    }), $scope.gridOptions = {
        data: "perContact",
        enablePaging: !0,
        showFooter: !0,
...
app.controller("HeaderController", ["$scope", "Authentication", "Menus", "sharedProperties", "Organizations", function($scope, Authentication, Menus, sharedProperties, Organizations) {
    $scope.authentication = Authentication, $scope.isCollapsed = !1, $scope.AllCompanies = Organizations.query(), "undefined" == typeof $scope.authentication.user.orgId ? ($scope.organization = [], $scope.organization.name = "Invoiceit") : $scope.organization = Organizations.get({
        organizationId: $scope.authentication.user.orgId
...
app.filter("selectFromSelected", function() {
    return function(incItems, value) {
        var out = [{}];
        if (console.log("value", value), value) {
            for (x = 0; x < incItems.length; x++) incItems[x].Value == value && out.push(incItems[x]);
            return out
        }
        return value ? void 0 : incItems
    }
}), app.controller("AuthenticationController", ["$scope", "$filter", "$http", "$window", "$location", "Authentication", "sharedProperties", "Users", "$stateParams", "licenseProperties", "rolesProperties", "Organizations", function($scope, $filter, $http, $window, $location, Authentication, sharedProperties, Users, $stateParams, licenseProperties, rolesProperties, Organizations) {
    $scope.authentication = Authentication, $scope.GlobalRoles = rolesProperties.all(), $scope.GlobalCompany = sharedProperties.orgLength(), $scope.GlobalUsers = Users.query(), $scope.GlobalOrganizations = sharedProperties.orgLength(), $scope.mySelections = [], $scope.sample = [], $scope.peruser = [], $scope.find = function() {
        $scope.allnames = $scope.Globalname, $scope.allCompanies = $scope.GlobalCompany, $scope.allRoles = $scope.GlobalRoles, $scope.Admincompany = Organizations.get({
            organizationId: $scope.authentication.user.orgId
        }), console.log("$scope.Admincompany--->", $scope.Admincompany), $scope.onlyUsers = Users.query(function(result) {
            for (var x = (result.length, 0); x < result.length; x++) result[x].orgId == $scope.authentication.user.orgId && $scope.peruser.push(result[x]);
            return $scope.peruser
        }), $scope.CheckLicenses = Users.query(function(result) {
            result.length, $scope.licenseLength = [];
            for (var x = 0; x < result.length; x++) result[x].orgId == $scope.authentication.user.orgId && $scope.licenseLength.push(result[x]);
            return console.log($scope.licenseLength.length > $scope.Admincompany.licenses ? "over" : "not over"), console.log("$scope.licenseLength.length", $scope.licenseLength.length), $scope.licenseLength.length
        }), $scope.InvoiceAdmin = [], $scope.InvoiceAdmin = "54756b6e089822ac1fcd0225" == $scope.authentication.user.orgId ? $scope.onlyUsers : $scope.peruser, $scope.filterOptions = {
            filterText: ""
        }, $scope.gridOptions = {
            data: "InvoiceAdmin",
            enablePaging: !0,
        ...
        $scope.Reloadvalue = !1, $scope.credentials.role = $scope.credentials.newrole._id, $scope.credentials.orgId = "54756b6e089822ac1fcd0225" == $scope.authentication.user.orgId ? $scope.credentials.company._id : $scope.Admincompany._id, $scope.credentials.displayName = $scope.credentials.firstName + $scope.credentials.lastName, $scope.credentials.username = $scope.credentials.firstName + $scope.credentials.lastName, $scope.credentials.password = $scope.credentials.firstName + $scope.credentials.lastName, $scope.sample.push({
            ...
            $scope.onlyUsers = Users.query(), $http.post("/auth/sendmailnewuser", $scope.sample).success(function() {}).error(function() {}), $scope.credentials = "", $scope.Reloadvalue = !0
...
}]), angular.module("users").directive("backButton", function() {
    return {
        restrict: "A",
        link: function(scope, element) {
            function goBack() {
                history.back(), scope.$apply()
            }
